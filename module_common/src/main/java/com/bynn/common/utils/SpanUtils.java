...
public final class SpanUtils {
  ...
  public static void init(@NonNull final Application application) {
    SpanUtils.mContext = application.getApplicationContext();
  }
   ...
   * @return {@link SpanUtils}
   */
  public SpanUtils setBullet(@ColorInt
  final int color, @IntRange(from = 0) final int radius, @IntRange(from = 0) final int gapWidth) {
  ...
  private void updateCharCharSequence() {
    ...
    if (iconMarginGapWidth != -1) {
      if (iconMarginBitmap != null) {
        ...
        mBuilder.setSpan(new CustomIconMarginSpan(mContext, iconMarginUri, iconMarginGapWidth, alignIconMargin), start, end, flag);
      } else if (iconMarginResourceId != -1) {
        mBuilder.setSpan(new CustomIconMarginSpan(mContext, iconMarginResourceId, iconMarginGapWidth, alignIconMargin), start, end, flag);
      }
    }
